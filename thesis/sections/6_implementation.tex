...
\todo{Write here about rest of the subsections}In Section~\ref{} \dots
...
\item Adjacent nodes have to agree on the labels of each edge. \todo{$\leftarrow$ rephrase this}
      We encode each adjacent node to not have different label for their shared edge.
      For each $e=(u, v)\in E$, for each label pair $(l_1, l_2), l_1, l_2 \in \Sigma, l_1 \neq l_2$, we require that at most one of the variables $L_{u,v,l_1}, L_{v,u,l_2}$ is true. \todo{We changed $L_{blabla}$, so update the above description!!!}
\item d
\item d
\item d
\end{enumerate}

% 1. Adjacent nodes need to agree on the edge's label. In other words, two adjacent nodes cannot label their shared edge differently.
\todo{}
% 2. Nodes need to have a valid labeling.
  ...
  \todo{}
  % 2.3 If a node has a labeling (a permutation of a configuration) then and only then the labeling must hold true.
    % 2.3.1 Active nodes
    \todo{}
    % 2.3.2 Passive nodes
    \todo{}
