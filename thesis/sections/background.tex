...
In the model, processes communicate by adding a message to message queue, whether it is a shared or a process specific, and the recipient process moves the message out (dequeues) from the message queue.
\cite{DBLP:books/el/leeuwen90/LamportL90}

Different message passing models are widely used in the field.
The models can varie in different details such as the size of the message queues \cite{DBLP:books/el/leeuwen90/LamportL90}, size of the messages \cite{peleg2000distributed} and how nodes get identified \cite{DBLP:conf/focs/Linial87}, if they are identified at all \cite{DBLP:conf/istcs/MayerNS95}.
We will discuss more about them in the following sections \ref{sec:port_number_model} and \ref{sec:local_model}.
The message passing model embodies a significant assumption that the message passing itself represents the dominant cost of an execution of an algorithm
\cite{DBLP:books/el/leeuwen90/LamportL90}.

As the message passing model can be defined in multiple ways, we use the definition that the LCL and PN use...\todo{revise}
\cite{DBLP:books/el/leeuwen90/LamportL90}.
