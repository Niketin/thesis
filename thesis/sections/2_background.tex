...
\begin{figure}[H]
  ...
  \hfill
  \subcaptionbox{The shortest path between nodes 1 and 5 in a graph $G$.
...
It is similar to running a computer program that contains multiple concurrent tasks, in a single computer, but in distributed computing there are higher level tasks that are distributed to multiple different computers.
...
\begin{figure}[H]
  \subcaptionbox{A PN network of two nodes, $a$ and $b$.
    Both $a$ and $b$ have a degree of 2, therefore 2 ports.
...
\begin{figure}[H]
  ...
  \hfill
  \subcaptionbox{
    An alternative representation of the PN network from Figure~\ref{fig:formal_pn2:a}
    \label{fig:formal_pn2:b}
  }%
    [.3\linewidth] {
    \centering
    \includegraphics[width=0.2\textwidth]{diagrams/formalizing_pn_network_diagram5.pdf}
  }
  \hfill
  \subcaptionbox{
    A PN network with loops and parallel connections.
    This kind of network cannot be shown in the alternative representation, because it does not support a loop that connects to the same port twice.
...
\begin{description}
  ...
  \item[Independent set] of a graph $G$ is a subset $V' \subseteq V$ of nodes where no two nodes are adjacent. That is, for each $u, v, \in V'$ such that $u \neq v$, an edge $(u, v)$ is not in $E$.
...
\begin{figure}[H]
  ...
  In Figures \ref{fig:distributed_graph_problems1:a} and \ref{fig:distributed_graph_problems1:b}, the blue nodes form a node labeling that denotes a solution.
...
\mcite{DBLP:conf/focs/Linial87, DBLP:journals/siamcomp/Linial92}.
...
\begin{definition} \label{def:covering_map}
  ...
  \begin{enumerate}
    ...
    \item $\phi$ preserves the degrees of nodes i.e.\ $\deg_N(v) = \deg_{N'}(\phi(v))$, for all $v \in V$.
...
\begin{figure}[H]
  ...
  \subcaptionbox{
    ...
    [.30\linewidth] {
    \centering
    \includegraphics[scale=0.48]{diagrams/covering_map_5b2.pdf}
  }
  \hfill
  \subcaptionbox{
...
Alternatively to $\Delta$-regular $\delta$-uniform hypertrees, we can think of the hyperedges as separate nodes \cite{DBLP:conf/podc/Olivetti20}.
...
We showed earlier examples of LCL problems in Section \ref{sec:lcl_problems}.
...
\begin{figure}[H]
  \subcaptionbox{
    ...
    The labels seen in the graph are part of a solution to 3-coloring problem.
...
One might wonder why are we interested in infinite $(\Delta, \delta)$-biregular trees and not finite trees?
