...
When an edge is defined as a pair, the order of the vertices in the edge matters, that is, $\forall v, w \in V, v \neq w: (v, w) \neq (w, v)$.
...
\subsection{Covering graph}
%TODO fix somehow
There exists a limitation to what problems can be solved in a deterministic PN model.
The limitation comes from a PN networks underlying graph being "somehow" symmetric.
\cite{DBLP:conf/focs/Linial87}
\cite{DBLP:journals/siamcomp/Linial92}

In this section we discuss about a graph theoretical concept that can help us determining the symmetries in an underlying graph.
The concept is called \emph{covering graph}
...
If $H$ is a covering of a connected graph $G$, then $H$ consists rather of multiple copies of $G$.
