...
Port number model (PN model) is a rather weak model of computation that inherits from the message passing model and does not use node identification.
...
All vertices of $G$ can be one-to-one mapped to nodes of $N$.
...
All nodes are considered identical, however nodes might have different degrees.
All nodes start execution simultaneously following the same deterministic PN-algorithm $A$.
...
Port-numbering network is a 3-element tuple $N = (V, P, p)$, where V is the set of nodes, 
