...
%Each vertex $v \in V$
An edge $e \in E$ can also be thought as a pair (2-tuple or 2-element ordered list) $e=(v, w), v, w \in V$, where vertices $v$ and $w$ are the endpoints of the edge $e$.
\end{definition}
For example, $G=(\{1, 2, 3\}, \{(1, 2),(1, 3),(2, 3),(3, 2)\})$, and visualized it looks like the graph on Figure \ref{fig:graph1:a}.

When the edge is defined as a pair, the order of the vertices in an edge matters, that is, $\forall v, w \in V, v \neq w: (v, w) \neq (w, v)$.
...
In the model, processes communicate by adding a message to a message queue, whether the queue is a shared or a process specific, and the recipient process moves the message out (dequeues) from the message queue.
\cite{DBLP:books/el/leeuwen90/LamportL90}
...
%We can further construct proofs of different theorems and so on. %TODO Fix this paragraph
...
%TODO It feels like this definition of ports should not depend on the degree as that depends on the previous definition of ports
...
We can use the concept of node labelings to represent a subset of nodes $X \subseteq V$ by using a node labeling $g: V \rightarrow \{0,1\}$ where $g(v) = 1$ indicates that $v \in X$ and $g(v) = 0$ indicates that $v \notin X$.

%TODO Mention edge labeling?
%We can also have an edge labeling with some additional work
...
\begin{description}
  ...
  \item[Maximal independent set] of a graph $G$ is an independent subset $V' \subseteq V$ of nodes such that there is no additional vertex $w \in V \backslash V'$ that can be included in $V'$ s.t. it stays as an independent set.
  A node labeling $f$ is a solution $f \in \Pi(N)$ if $f$ encodes an independent set of nodes of the underlying graph $G$ of $N$.
  \item[3-coloring] of a graph $G$ is 3 subsets $C_1, C_2, C_3 \subset V$ such that the sets are subsets of $V$, and each node is exactly in one subset.
  Additionally it is required that the subsets are independent sets.
  A node labeling $f$ is a solution $f \in \Pi(N)$ if $f$ encodes a 3-coloring of the underlying graph $G$ of $N$.
\end{description}
...
\todo{This is probably needed}
%% HERE IS SOME DRAFT TEXT WRITTEN ABOUT THE ABOVE TOPIC.
...
%
%When we say that a distributed algorithm $A$ solves a problem $\Pi$ on a graph family $\mathcal{F}$ given problem $\Pi'$ if the execution

\subsection{Covering maps}
\todo{This is probably needed because we basically talk about covering maps when we use the word "lift" on Section \ref{sec:algorithm}.}
...
\todo{Talk about the benefits on the local model when compared to PN model (symmetry breaking things). ONLY IF NEEDED}
