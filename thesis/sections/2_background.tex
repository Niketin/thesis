...
The LOCAL model is another message passing model that is used in the field of distributed computation. It is similiar to the PN model, hence most of the definition is already done.
The difference is that the graph problem $\Pi'$ given as an input is always predetermined.
A solution to problem $\Pi'$ is a node labeling $\operatorname{id}\colon V \rightarrow \{1,2,\dotsc,|V|^c\}$, where each node has a unique label \cite{HirvonenSuomelaDistAlg2020}.
The value $c$ is a positive constant greater than 1 \cite{HirvonenSuomelaDistAlg2020}, but in older literature it has been defined as 1 \cite{DBLP:journals/siamcomp/Linial92}.
...
In the LOCAL model, each node has a unique identifier, therefore an algorithm developed for the LOCAL model can utilize these identifiers to break the symmetry and avoid the limitation of PN model.

\subsection{LCL problems} \label{sec:lcl_problems}

\cite{DBLP:journals/siamcomp/NaorS95}

In this work, we are specially interested in LCL problems in $(b,a)$- or $(a,b)$-regular graphs.
The reason is ...
