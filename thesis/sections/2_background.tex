...
\todo{Discuss about the solving of $\Pi$ as in the paper \cite{DBLP:journals/corr/abs-2105-05574} }



\todo{Definition of LCL (in the round eliminator formalism)}
...
\subsubsection{LCL problems in biregular graphs}
...
In the formalism, LCL problems are defined for \emph{infinite $\Delta$-regular tree} graphs.
\todo{
Input labels are considered to be part of the given graphs.
This way whe problem is separated from the inputs and graph topology?\cite{DBLP:conf/podc/Brandt19}
}
...
\todo{
"Nodes are required to write a label on each incident edge => each edge will be labelled with 2 labels, one for each endpoint" \cite{DBLP:conf/podc/Olivetti20}
}

\todo{
  " there is a set of acceptable output configurations for the two endpoints of an edge, and the global output is correct if and only if the configuration for each edge is acceptable."
  \cite{DBLP:conf/podc/Brandt19}
}

\todo{
  "Also, we can use this encoding
to define problems on bipartite 2-colored graphs, by mapping white
and black nodes to nodes and hyperedges. In this case, we will refer
to the node constraint as white constraint and to the hyperedge
constraint as black constraint."
\cite{DBLP:conf/podc/Olivetti20}
}

We call the partition $A$ of the graph as the \emph{active} partition and the partition $B$ as the \emph{passive} partition.
...
In this work, we are specially interested in LCL problems in $b$-regular graphs.
