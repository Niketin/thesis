...
The LOCAL model is another message passing model that is used in the field of distributed computation. It is a variant of the PN model, hence most of the definition is already done.
...
In the LOCAL model, each nod   LOCAL model can utilize these identifiers to break the symmetry and avoid the limitation of PN model.

\clearpage % TODO REMOVE clearpage
...
The paper laid the groundwork that was needed for researching the locality of distributed graph problems.
...
\begin{description}
  \item[$k$-coloring] \todo{text}
  \item[Maximal independent set] \todo{text}
  \item[Minimal vertex cover] \todo{text}
\end{description}
...
\todo{Talk about LCL's in general}

\todo{Define LCL for vertex or edge labels}
...
In this work, we are specially interested in LCL problems in $(b,a)$-regular graphs.
The reason is ...
