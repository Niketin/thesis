...
The models can varie in different details, such as in the size of the message queues \cite{DBLP:books/el/leeuwen90/LamportL90}, in the size of the messages \cite{peleg2000distributed} and on how nodes get identified \cite{DBLP:conf/focs/Linial87}, if they are identified at all \cite{DBLP:conf/istcs/MayerNS95}.
As the message passing model itself can be defined in multiple ways \cite{DBLP:books/el/leeuwen90/LamportL90}, we want to use a definition from which the relevant models also inherit from.
We will discuss more about the most relevant models in the following sections \ref{sec:port_number_model} and \ref{sec:local_model}.
