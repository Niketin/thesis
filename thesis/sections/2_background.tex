...
Listing the locally consistent labelings of each problem would result in too large lists, hence we give only a brief description of the elements.
\begin{description}
  ...
  The set $C$ contains all the possible 1-ball graphs of a given graph family with all possible combinations of output labels such that if the node $s$ (the center node) is $1$, then all adjacent nodes are labelled with $0$.
...
In this work, we are specially interested in LCL problems in $(b,a)$-biregular graphs.
The reason is ...
