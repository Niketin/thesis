...
% Using Corollary \ref{cor:lcl_solvability:contrapositive}, we can see that for each network $A_i, 1 \leq i \leg m$, there exists some network $N_j, 1 \leg j \leg n$ where the algorithm cannot find a solution, denoted by value ``N''.
...
%        \node (5) [below = of 4] {Theorem \ref{thm:algorithm:from_pn_to_local:lcl_unsolvability}};
...
Now with Lemmas \ref{lem:lcl_unsolvability:from_multiple_to_simple}, \ref{lem:problem_unsolvability_in_graphs}, and \ref{lem:from_finite_to_infinite}, the output graphs from our algorithm imply the unsolvability in infinite $(\Delta, \delta)$-biregular tree networks.
...
\begin{figure}[H]
   ...
   \begin{tikzpicture}[]
       ...
       \node (4) [below = of 3] {$\exists\mathcal{O}(1)$ in PN in trees};
       \node (5) [below = of 4] {$\exists\mathcal{O}(1)$ in PN in general graphs};
       \node (6) [below = of 5] {Solvable in PN in general graphs};
       ...
       \draw[->] (3) -- (4) node [pos=0.5,left,font=\footnotesize] {Lemma \ref{lem:algorithm:from_pn_to_local:o1_oi_trees_imply_o1_pn_trees}};%
       %\draw[->] (4) -- (5) node [pos=0.5,left,font=\footnotesize] {Lemma \ref{lem:algorithm:from_pn_to_local:o1_po_trees_imply_o1_pn_trees}};
       \draw[->] (4) -- (5) node [pos=0.5,left,font=\footnotesize] {Lemma \ref{lem:algorithm:from_pn_to_local:o1_pn_trees_imply_o1_pn_gen}};
       \draw[->] (5) -- (6) node [pos=0.5,left,font=\footnotesize] {Lemma \ref{lem:algorithm:from_pn_to_local:o1_pn_trees_imply_pn_gen}};
    \end{tikzpicture}
    \caption{Here are all the implications we show in this section.
    Each implication is a step towards showing Lemma \ref{lem:lcl_solvability:constant_local_imply_constant_pn}.
...
% For the next two lemmas, we need to introduce another model called \emph{port numbering + orientation} (PO).
% In the model, ports are numbered like in the PN model and additionally edges have an orientation \cite{DBLP:journals/dc/GoosHS17}.
...
% %%%% O(1) PO in trees
% % -> O(1) PN in trees
% \begin{lemma} \label{lem:algorithm:from_pn_to_local:o1_po_trees_imply_o1_pn_trees}
%     If an LCL problem is solvable in constant time in trees in the PO model, then the problem is solvable in constant time in trees in the PN model.
% \end{lemma}
% \begin{proof}
%     \todo{}
% \end{proof}
...
%%%% O(1) PN in trees
% -> O(1) PN in general
\begin{lemma} \label{lem:algorithm:from_pn_to_local:o1_pn_trees_imply_o1_pn_gen}
...
\end{lemma}
\begin{proof}
    \todo{Unfold general graph to infinite tree. Apply tree algorithm. The algorithm only considers a constant radius neighborhood at each node and maps it to output. By covering map argument, it properly labels the general graph.}
\end{proof}
...
%%%
% Here I write notes from our meeting from Thu. 12.5.2022 15:15 - 15-45

% 1st prove the case of cycles.
% Then we can start to see how we can generalize it.

% Start assuming that problem is solvable in constant time in the LOCAL model.
% This directly implies also constant time solution in the PN model.
% We need to show this.

% Constant time local in trees implies constant time pn in trees.

% final step would be to show that 

% If you have constant time PN algorithm for trees, then it also implies you can solve this problem in port numbering model in any graph for constant time.

% We have 10-time algorithm for trees in PN. Then use the same algorithm for any PN network such that each node unfolds their local neighbourhood at distance 10
