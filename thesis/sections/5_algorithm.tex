...
The Definitions \ref{def:lcl_solvability} and \ref{def:lcl_solvability:contrapositive} are equivalent. The latter one might be more difficult to understand, but we find it more fitting to discuss unsolvability rather than solvability.

We have illustrated an example of an unsolvable LCL problem $\Pi$ in Table \ref{tbl:unsolvable_lcl:1}.
In the example, there are only finite number of networks ($n$-many) and algorithms ($m$-many) for the sake of simplicity.
For each algorithm, there is at least one network where the algorithm fails to solve problem $\Pi$, indicated by ``N'' (No).
On the other hand, ``Y'' (Yes) means that the algorithm can solve the problem in the network.

In Table \ref{tbl:unsolvable_lcl:2}, we show an example that is a special case of \ref{def:lcl_solvability:contrapositive}.
We use the same notations as in Table \ref{tbl:unsolvable_lcl:1}.
In the example, the problem $\Pi'$ is also unsolvable, but this time \emph{no algorithm} can solve it in network $N'_{n'}$.
...
\begin{table}[H]
    ...
    \caption{
        ...
        Each algorithm fails to solve $\Pi$ in at least some network.
    }
    ...
    \caption{
        Unsolvable LCL problem $\Pi'$.
        Each algorithm fails to solve $\Pi'$ at least in network $N'_{n'}$.
    }
    \label{tbl:unsolvable_lcl:2}
    }
\end{table}

Not every unsolvable LCL problem necessarily fall within the case shown in Table \ref{tbl:unsolvable_lcl:2}, but nevertheless we are interested in it.
Thus, we come up with Lemma \ref{lem:lcl_unsolvability}.
...
We assume that $\Pi$ is not solvable in $G$, and $\Pi$ is solvable in some $N$ that has $G$ as its underlying graph.
This means that there exists a valid labeling for $N$, therefore there must exist a valid labeling for $G$.
This is contradicting with the assumption that $\Pi$ is not solvable in $G$.
\end{proof}
...
Now we have completed showing our algorithm.
This means that we have now given our answer to Research question \ref{research_question:1}.
What is left in this section, is to answer to Research question \ref{research_question:2}.
...
\todo{theorem does not mention multiple connections at all. what now?}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[]
        \node (1) [] {Lemma \ref{lem:lcl_unsolvability:2}};
        \node (2) [right = of 1] {Lemma \ref{lem:lcl_unsolvability:3}};
        \node (3) [below = of $(1)!0.5!(2)$] {Lemma \ref{lem:lcl_unsolvability:4}};
...
\begin{lemma} \label{lem:lcl_unsolvability:3}
...
% \caption{\todo{Illustration of k-lift using Theorem \ref{lem:lcl_unsolvability:3}}}
% \label{fig:duck2}
%\end{figure}
